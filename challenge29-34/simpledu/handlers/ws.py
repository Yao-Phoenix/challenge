...
class Chatroom(object):
    ...
    def send(self, client, data):
        ...
        try:
            # python3 中接受到的消息是二进制的, 使用 decode 函数转化为字符串
...
@ws.route('/send')
def inbox(ws):
    # 使用 flask-sockets, ws 链接对象会被自动注入到路由处理函数,该处理函数用来处理前端发过来的消息
    # 注意下面的 while 循环, 里面的 receive() 函数实际是在阻塞运行的, 直到前端发送消息过来
    # 消息会被放入到 chat 频道, 也就是我们的消息队列中, 这样一直循环, 知道 websocket 连接关闭
    while not ws.closed:
        message = ws.receive()
...
@ws.route('/recv')
def outbox(ws):
    # 该函数用来注册客户端连接, 并在 Chatroom 中将从其他客户端接收到的消息大送给这些客户端
    chat.register(ws)
