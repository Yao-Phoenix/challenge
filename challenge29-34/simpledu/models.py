...
class User(Base, UserMixin):
    ...
    # 就可以了, 数值之间设置了 10 的间隔是为了方便以后加入其他角色
    ROLE_USER = 10
    ROLE_STAFF = 20
    ROLE_ADMIN = 30
    ...
    # 默认情况下, sqlalchemy 会以字段名来定义列名, 但这里是 _password, 所以明确
    # 指定数据库表列名为 password
    ...
    def password(self, orig_password):
        ''' Python 风格的 setter, 这样设置 user.password 就会自动为
        password 生成哈希值存入 _password 字段
        '''
